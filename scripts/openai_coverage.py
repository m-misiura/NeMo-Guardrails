...
def main():
    ...
    if args.check_breaking:
        print("[1/2] Breaking changes (HEAD vs working tree):", end=" ")
        if _check_breaking():
            print("none")
        else:
            sys.exit(1)
    ...
    if args.check_breaking:
        print(f"[2/2] Conformance (OpenAI v{report['openai_version']} vs fern/openapi.yml):", end=" ")

    if args.check_regression and prev_changes is not None:
        if n_changes > prev_changes:
            print(f"REGRESSION {prev_changes} -> {n_changes} changes (+{n_changes - prev_changes})")
            print("To update the baseline: python scripts/openai_coverage.py --update")
            sys.exit(1)
        elif n_changes < prev_changes:
            print(f"improved {prev_changes} -> {n_changes} changes (-{prev_changes - n_changes})")
        else:
            print(f"{n_changes} changes (unchanged)")
    elif args.check_breaking:
        print(f"{n_changes} changes")

    if not args.check_breaking:
